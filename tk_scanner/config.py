...
class Config:
    ...
    # Базовые настройки
    lookback: int = 50  # Сколько последних свечей анализировать для обнаружения TK-баров
    ...
    # Параметры стратегии
    base_zone_percent: float = 0.5  # Базовый процент зоны интереса — насколько цена должна измениться, чтобы свеча считалась значимой (0.2 = 20%)
